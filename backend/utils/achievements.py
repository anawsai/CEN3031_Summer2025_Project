def check_task_achievements(user_id, total_tasks_completed, service_supabase):
    ...
    for achievement_code, required_count in task_milestones:
        if total_tasks_completed >= required_count:
            existing = service_supabase.table('user_achievements').select('id').eq(
                'user_id', user_id
            ).eq('achievement_id', 
                service_supabase.table('achievements').select('id').eq('code', achievement_code).execute().data[0]['id']
            ).execute()
            
            if not existing.data:
                achievement = service_supabase.table('achievements').select('*').eq(
                ...
                xp_response = service_supabase.table('user_xp').select('*').eq('user_id', user_id).execute()
                if xp_response.data:
...
def get_user_stats(user_id, service_supabase):
    ...
    tasks_response = service_supabase.table('Tasks').select('id').eq(
        'user_id', user_id
    ).eq('completed', True).execute()
    total_tasks = len(tasks_response.data) if tasks_response.data else 0
    
    focus_response = service_supabase.table('focus_sessions').select('id').eq(
        'user_id', user_id
    ).eq('completed', True).execute()
