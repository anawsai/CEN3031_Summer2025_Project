...
const PomodoroTimer = ({
      ...
      pomodoroAPI
        ...
        .then((response) => {
          ...
          if (response.achievements_earned && setNotificationQueue) {
            const achievementNotifications = response.achievements_earned.map(
              (achievement) => ({
                type: 'achievement',
                ...achievement,
              })
  ...
  }, [timeLeft, sessionId, workDuration, refreshXP, setNotificationQueue]);
  ...
  return (
    <div>
      ...
      {isPaused && (
        <div
          style={{ textAlign: 'center', color: '#888', marginBottom: '10px' }}
        >
          Paused
        </div>
      )}
